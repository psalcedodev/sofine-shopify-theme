$(document).ready(function () {
  ...
  function get_price_only(price) {
    ...
    if ($split.length > 1) {
      get_currencies = isNaN(parseInt(price_convert.substring(1, 0)))
        ? $split[1]
        : $split[0];
  ...
  if ($split.length > 1) {
    get_currencies = isNaN(parseFloat(get_currencies.substring(1, 0)))
      ? $split[0] + ";"
      : $split[1] + ";";
  } else {
    get_currencies = isNaN(parseFloat(get_currencies.substring(1, 0)))
      ? get_symbol(get_currencies)
      : get_currencies.substring(1, -1);
  }
    ...
    jQuery(document).ajaxComplete(function (event, xhr, settings) {
      ...
      if (index == paramTotal) {
        if (paramTotal > paramLimit) {
          ...
          loadMoreHTML =
            loadMoreHTML +
            "<div id='loadMore' class='bn_button viewall text-center' style=''>";
          loadMoreHTML =
            loadMoreHTML + "<a href='javascript:void(0);'>Load More</a>";
  ...
  function initFilterPrice() {
    $("div[data-range-price]").slider({
      ...
      values: [
        get_price_only(min_price_slide),
        get_price_only(max_price_slide),
      ],
      slide: function (event, ui) {
        ...
        var $split = get_currencies.split(";");
        if ($split.length > 1) {
          get_currencies = isNaN(parseInt(get_currencies.substring(1, 0)))
            ? $split[0] + ";"
            : $split[1] + ";";
        } else {
          get_currencies = isNaN(parseInt(get_currencies.substring(1, 0)))
      ...
      change: function (event, ui) {
        jQuery("body").addClass(
          "ajax_loading"
        ); /* Hiển thị loading khi filter ajax*/
        ...
        var vHtml =
          ""; /* Khởi tạo biến html = rỗng để chuẩn bị cho việc append các sản phẩm vào khu vực kết quả */
        var vView =
          $(".js_cat_view.active").data("col") > 0
            ? "grid"
            : "list"; /* Lấy view từ collection view có trạng thái active với data-col = 0 là list*/
        ...
        $("div[data-container-cat]").addClass(
          "js_product_ajax"
        ); /* Thêm class để điều khiển phân trang */
        $(
          ".products-result-count"
        ).empty(); /* Xoá biến hiển thị số kết quả phân trang */
        $(
          ".js_paginate_ajax"
        ).empty(); /* Xoá dữ liệu phần tử hiển thị kết quả phân trang */

        if ($(".js_cat_view.active").length <= 0) {
          /* Nếu không tồn tại collection view thì gán view = grid */
          vView = "grid";
        }
        ...
        $.each(json_collection_filter, function (paramIndex, paramElement) {
          var current_price = convert_currency(
            paramElement.variants[0].price,
            ""
          ); /* Lấy giá hiện thời của từng sản phẩm */
          current_price =
            get_price_only(current_price); /* Chỉ lấy số của giá */
          if (current_price <= vMaxPrice && current_price >= vMinPrice) {
            /* Kiểm tra giá sản phẩm có thoả mãn điệu kiện lọc */
            ...
            xhr = $.ajax({
              ...
              success: function (data) {
                ...
                setTimeout(function () {
                  tippy(
                    ".product-item .js_add_to_cart_button, .product-item .js-compare, .product-item .js-wishlist, .product-item .js_quick_view,.product-item-style2.small .js_add_to_cart_button, .product-item-style2 .js-compare, .product-item-style2 .js-wishlist, .product-item-style2 .js_quick_view",
                    {
                      placement: "top",
        ...
        if (!vHasProduct) {
          /* Nếu duyệt hết mảng sản phẩm mà không tìm được sản phẩm thoả mãn điều kiện */
          vHtml +=
            '<div class="shopify-info no-products"><i class="flash fa fa-exclamation-triangle" aria-hidden="true"></i>There are no products that match your price</div>';
        }
        $("#collection-product").append(
          vHtml
        ); /* Append giao diện kết quả sản phẩm vào khu vực kết quả lọc */

        loadMoreProduct(limit_product, vIndex);

        if ($(".shopify-product-reviews-badge").length > 0) {
          /*Reinit lại app product review */
          SPR.registerCallbacks();
          SPR.initRatingHandler();
          SPR.initDomEls();
          SPR.loadProducts();
        ...
        window.setTimeout(function () {
          jQuery("body").removeClass(
            "ajax_loading"
          ); /* Sau khi đã hiển thị kết quả lọc thì đóng loading */
        }, 1000);
      },
    });
  }
  ...
  $(document).on("shopify:section:load", function () {
    /* Khi section được load trong admin */ initFilterPrice(); /* Gọi lại hàm lọi với price*/
  });
